...
def _gqa_share_sparse_fwd_kernel(
    ...
    PAGE_RUNS: tl.constexpr,   # runs per block (SPAN * RUNS == BLOCK_SIZE_K)
):
    ...
    for j in range(real_q_loop):
        ...
        for i in range(real_topk):
            ...
            if PAGED_KV:
                ...
                # run replaces the BLOCK_SIZE_K-entry index gather.
                if PAGE_RUNS == 1:
                    # page_size >= block_size: the whole block is inside one page.
                    # Kept as its own branch because the general form below carries
                    # a [RUNS, SPAN] broadcast + reshape that costs ~1% here.
                    base_slot = tl.load(
...
@torch.no_grad()
def flash_prefill_with_gqa_share_sparse_paged(
    q: torch.Tensor,
    ...
    """Paged-fast-path copy of ``flash_prefill_with_gqa_share_sparse``.
    ...
    ``req_to_token`` lookup *per run* instead of a ``block_size_k``-entry gather:
    ...
    fall back to the original per-token gather.
    """
    ...
    # Short runs are excluded: at PAGE_SPAN 8 the [RUNS, SPAN] reshape costs more
    # than the gather it replaces (measured 0.96x, i.e. a regression), while
    # PAGE_SPAN 16 is already a 1.19x win. See
