...
def _gqa_share_sparse_fwd_kernel(
    ...
    for j in range(real_q_loop):
        ...
        for i in range(real_topk):
            ...
            # div/mod instruction, so `% max_slots` on a BLOCK_SIZE_K-wide vector
            # expands into a long reciprocal sequence per lane. A select is
            # equivalent for slots in [-max_slots, max_slots).
...
@torch.no_grad()
def flash_prefill_with_gqa_share_sparse_nomod(
    q: torch.Tensor,
