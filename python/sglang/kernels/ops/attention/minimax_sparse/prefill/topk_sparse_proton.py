...
def _gqa_share_sparse_fwd_kernel(
    ...
    for j in range(real_q_loop):
        ...
        for i in range(real_topk):
            ...
            with pl.scope("1_blockid_load"):
                c = tl.load(t_ptr_j).to(tl.int32) * BLOCK_SIZE_K
                t_ptr_j = t_ptr_j + stride_tk
            # paged load K via req_to_token: pos -> slot -> k_cache
            with pl.scope("2_slot_resolve"):
                pos = c + off_n
                pos_mask = pos < seq_len
                if PAGED_KV:
                    # page_size == block_size, so this block is exactly one physical
                    # page and its slots are consecutive. One scalar lookup replaces
                    # the BLOCK_SIZE_K-entry index gather, and `slots` becomes affine
                    # in off_n so Triton can emit a strided vector load rather than a
                    # general gather.
                    base_slot = tl.load(
            ...
            with pl.scope("6_v_load"):
                ...
                if IS_FP8:
                    ...
                    v = v.to(q.dtype)
            with pl.scope("7_pv_dot_and_stats"):
                p = p.to(v.dtype)
                acc_o += tl.dot(p, v)
                # update statistics
                m_i = m_ij
...
@torch.no_grad()
def flash_prefill_with_gqa_share_sparse_proton(
    q: torch.Tensor,
    ...
    Numerically identical to the original. When ``page_size == block_size_k`` a
    selected block maps onto exactly one physical page, so the inner loop needs
    one ``req_to_token`` lookup instead of a ``block_size_k``-entry gather.
    Falls back to the original path for any other page size.
    """
    ...
    # See the decode copy: the fast path needs block and page to partition
    # logical positions identically, and the pool to be a whole page count.
    paged_kv = page_size == block_size_k and max_slots % page_size == 0
