...
def _gqa_share_sparse_fwd_kernel(
    ...
    for j in range(real_q_loop):
        ...
        for i in range(real_topk):
            ...
            # Exactly equivalent to `(slots + max_slots) % max_slots` for slots in
            # [-max_slots, 2*max_slots) -- i.e. it still wraps out-of-range values --
            # but without a runtime integer modulo, which GPUs lack an instruction
            # for and which expands into a per-lane reciprocal sequence.
...
@torch.no_grad()
def flash_prefill_with_gqa_share_sparse_safemod(
    q: torch.Tensor,
