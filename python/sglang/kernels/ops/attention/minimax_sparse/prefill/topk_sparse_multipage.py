...
def _gqa_share_sparse_fwd_kernel(
    ...
    PAGE_SPAN: tl.constexpr,   # min(page_size, block_size): tokens per run
    PAGE_RUNS: tl.constexpr,   # block_size // PAGE_SPAN: runs per block
):
    ...
    for j in range(real_q_loop):
        ...
        for i in range(real_topk):
            ...
            if PAGED_KV:
                # A block starts at a multiple of block_size, so when page_size and
                # block_size divide one another the block decomposes into PAGE_RUNS
                # whole, page-aligned runs of PAGE_SPAN consecutive slots. Look up
                # one slot per run instead of one per token:
                #   page >= block -> 1 run  of block_size   (the single-lookup case)
                #   page <  block -> block/page runs of page_size
...
@torch.no_grad()
def flash_prefill_with_gqa_share_sparse_multipage(
    q: torch.Tensor,
    ...
    # Fast path needs the block to split into whole page-aligned runs, i.e.
    # page_size and block_size must divide one another (both are powers of two
    # in practice, so this nearly always holds).
