...
# Smallest per-page run the multi-run path is worth taking. Below this the
# [RUNS, SPAN] reshape costs more than the gather it replaces.
MIN_PAGE_SPAN = 16
...
def _gqa_share_sparse_decode_kernel(
    ...
    PAGE_SPAN: tl.constexpr,   # tokens per consecutive run
    PAGE_RUNS: tl.constexpr,   # runs per block (SPAN * RUNS == BLOCK_SIZE_N)
):
    ...
    for _ in tl.range(chunk_start_topk, chunk_end_topk):
        ...
        if PAGED_KV:
            ...
            # page-aligned runs of PAGE_SPAN consecutive slots. One lookup per run
            # replaces the BLOCK_SIZE_N-entry index gather.
            if PAGE_RUNS == 1:
                # page_size >= block_size: the whole block is inside one page.
                # Kept as its own branch because the general form below carries a
                # [RUNS, SPAN] broadcast + reshape that measurably costs here.
                base_slot = tl.load(
                ...
                # page_size < block_size: PAGE_RUNS lookups, one per page.
                ...
                bases = (bases + max_slots) % max_slots
                slots = tl.reshape(
...
def flash_decode_with_gqa_share_sparse_paged(
    ...
    """Paged-fast-path copy of ``flash_decode_with_gqa_share_sparse``.

    Numerically identical to the original -- bit-exact, not just close.

    When page and block sizes divide one another, a selected block covers whole
    page-aligned runs of consecutive slots, so the inner loop needs one
    ``req_to_token`` lookup *per run* instead of a ``block_size``-entry gather:

        page_size >= block_size   ->  1 lookup   (e.g. page 128, 256, 512)
        page_size <  block_size   ->  block/page lookups (e.g. page 64 -> 2)

    Runs shorter than ``MIN_PAGE_SPAN`` and page sizes that divide neither way
    fall back to the original per-token gather, so this is always safe to call.
    """
    ...
    # The fast path needs a selected block to cover whole, page-aligned runs, so
    # its slots are consecutive within each run. A block starts at a multiple of
    # block_size, so that holds exactly when page and block sizes divide one
    ...
    # Short runs are excluded: below MIN_PAGE_SPAN the [RUNS, SPAN] reshape costs
    # more than the gather it replaces. See ../prefill/topk_sparse_paged.py and
    # results/stage1_slot_compute/ANALYSIS.md.
