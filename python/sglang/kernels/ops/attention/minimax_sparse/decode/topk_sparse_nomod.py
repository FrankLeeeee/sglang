...
def _gqa_share_sparse_decode_kernel(
    ...
    for _ in tl.range(chunk_start_topk, chunk_end_topk):
        ...
        # Same guard without a runtime integer modulo. GPUs have no integer
        # div/mod instruction, so `% max_slots` on a BLOCK_SIZE_N-wide int64
        # vector expands into a per-lane reciprocal sequence. Equivalent for
        # slots in [-max_slots, max_slots); does NOT wrap slots >= max_slots.
...
@torch.no_grad()
def flash_decode_with_gqa_share_sparse_nomod(
